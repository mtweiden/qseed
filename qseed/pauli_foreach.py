...
#                block_data['recommended_seeds'] = recommendations
        ...
        # Do encoding in parallel
        start = time()
        encodings = await get_runtime().map(
            do_encoding,
            subcircuits
        )
        stop = time()
        ...
        seeded_block_datas = []
        start = time()
        for encoding, subcircuit, block_data in zip(encodings, subcircuits, block_datas):
            if encoding is None:
                seeded_block_datas.append(block_data)
                continue
            connectivity = self.detect_connectivity(subcircuit)
            model_out = self.models[connectivity](tensor(encoding).float())
            recommendations = self.decode(model_out, connectivity)
            block_data['recommended_seeds'] = recommendations
            seeded_block_datas.append(block_data)
        stop = time()
